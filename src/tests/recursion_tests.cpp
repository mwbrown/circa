...
void preserve_subroutine_local_template(std::string input)
{
    test_that_f_returns_1("preserve_subroutine_local",
            "def f(int level) -> any { " + input + " if level==0 { f(1) } return a }");
}
...
void preserve_subroutine_local()
{
    ...
    preserve_subroutine_local_template("a = 'x'; if true { a = level+1 }");
    preserve_subroutine_local_template("a = level+1; for i in [] { a = 'x' }");
    preserve_subroutine_local_template("a = 'x'; for i in [1 2] { a = level+1 }");
}
...
void preserve_if_local()
{
    ...
    preserve_if_local_template("a = 'x' if true { a = level+1 }");
}
...
void preserve_for_local()
{
    ...
    preserve_for_local_template("a = for j in [2] { a = 1 }");
}
